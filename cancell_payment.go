...
func CancelPayment(config *Config) error {
	...
	return retry.Retry(func() error {
		...
		req, err := http.NewRequest("DELETE", fmt.Sprintf("https://www.adidas.pl/api/chk/baskets/%s/payment_methods/PAYPAL", config.Resources.BasketID), nil)
		...
		if res.StatusCode != 200 && res.StatusCode != 404 && res.StatusCode != 204 {
			config.DefaultConfig.Log.StatusCodeErrorDelay(res.Status)
			return definederrors.ERROR_PLACEHOLDER
		}

		config.DefaultConfig.Log.Green("Payment cancelled successfully!")
